...
library Permit2Lib {
    ...
    function transferFrom2(address token, address from, address to, uint256 amount) internal {
        // Generate calldata for a standard transferFrom call.
        bytes memory inputData = abi.encodeWithSelector(0x23b872dd, from, to, amount);
        ...
        if (!success) PERMIT2.transferFrom(from, to, amount.safeCastTo160(), token);
    }
    ...
    function permit2(
        ...
        bytes memory inputData = abi.encodeWithSelector(0x3644e515);
        ...
        if (success) {
            ...
            inputData = domainSeparator == DAI_DOMAIN_SEPARATOR
                ...
                ? abi.encodeWithSelector(0x8fcbaf0c, owner, spender, EIP2612(token).nonces(owner), deadline, true, v, r, s)
...
abstract contract Permit2 {
    ...
    function permit(address owner, PermitSingle memory permitSingle, bytes calldata signature) external virtual;
}

abstract contract EIP2612 {
    function nonces(address account) external view virtual returns (uint256);
}
